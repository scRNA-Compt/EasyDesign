...
from easyDesign.utils import guide
...
class PrimerSearcher(guide_search.GuideSearcherMinimizeGuides):
    ...
    def check_continuous(self, primer_seq):
        primer_seq = str(primer_seq)
        if primer_seq.__contains__(self.require_a) or primer_seq.__contains__(self.require_t) \
                or primer_seq.__contains__(self.require_c) or primer_seq.__contains__(self.require_g):
            return True
        return False

    def check_5_prime(self, primer_seq):
        primer_seq = str(primer_seq)
        sub_primer = primer_seq[:3]
        if sub_primer.__contains__("G"):
            return True
        return False

    def check_3_prime(self, primer_seq):
        primer_seq = str(primer_seq)
        sub_primer = primer_seq[len(primer_seq) - 3:]
        if not sub_primer.__contains__("G") and not sub_primer.__contains__("C"):
            return True
        return False

    def simple_check_content(self, primers):
        for primer_seq in primers:
            if self.check_continuous(primer_seq):
                return False
            if self.check_5_prime(primer_seq):
                return False
            if self.check_3_prime(primer_seq):
                return False
        return True
    ...
    def find_primers(self, max_at_site=None):
        ...
        for cover in self._find_guides_for_each_window(
            ...
            if max_at_site is not None and num_primers > max_at_site:
                continue
            if self.simple_check_content(primers_in_cover) is False:
                continue
