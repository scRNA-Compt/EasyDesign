...
allowInsertDelete = True #yh: for sgRNA insert/delete (e.g. ACTGACTG -> ACTACTG), use 5 letters, got 'ACT-ACTG'
...
class Predictor:
    ...
    def __init__(self, classification_model_path, regression_model_path,
        ...
        if classification_guide_length != regression_guide_length:
            raise Exception(("Classification and regression models should "
                "have been trained with the same guide_length, but they differ: " + str(classification_guide_length) + " !=reg " + str(regression_guide_length)))
    ...
    def _model_input_from_nt(self, pairs):
        ...
        x = np.empty((len(pairs), l, dictsize*2), dtype='f')
    ...
    def _regress(self, pairs_onehot):
        ...
        #print ("in easyDesign/utils/predict_activity.py:_regress: pairs_onehot = ", pairs_onehot, ", pred_regression_score = ", pred_regression_score) #yh
        ...
        #print ("in easyDesign/utils/predict_activity.py:_regress: after bound and shift, pred_regression_score = ", pred_regression_score) #yh
        ...
        for target_with_context, guide in pairs:
            if len(guide) != self.guide_length:
                raise ValueError(("For the models being used, length of "
                    "guide must be %d, not %d") %  (self.guide_length, len(guide)))

        # Create one-hot encoding of pairs
        pairs_onehot = self._model_input_from_nt(pairs)
        #print ("in easyDesign/utils/predict_activity.py:_run_models_and_memoize: pairs = ", pairs, ", pairs_onehot = ", pairs_onehot) #yh
