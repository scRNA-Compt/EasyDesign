...
from easyDesign import alignment
from easyDesign import guide_search
from easyDesign.prepare import align
from easyDesign.prepare import ncbi_neighbors
from easyDesign.prepare import prepare_alignment
from easyDesign import primer_search
from easyDesign.specificity import alignment_query
from easyDesign import target_search
from easyDesign.utils import log
from easyDesign.utils import predict_activity
from easyDesign.utils import seq_io
from easyDesign.utils import year_cover
from easyDesign.utils import mutate
from easyDesign.utils.version import get_project_path, get_latest_model_version
...
def reverse_complement(seq):
    """reverse and complement for input seq

    Args:
        seq: a list or a pair of sequence

    Returns:
        The result as you input "ATC"(['A', 'T', 'C']) and get the result "GAT"(['G', 'A', 'T'])
    """
    complement = {'A': 'T', 'C': 'G', 'G': 'C', 'T': 'A', 'K': 'M', 'M': 'K', 'R': 'Y', 'Y': 'R', 'S': 'W', 'W': 'W',
                  'B': 'V', 'V': 'B', ' H': 'G', 'D': 'C', 'X': 'N', 'N': 'N', '-': '-'}
    if type(seq) == str:
        assert seq != ""
        bases = list(seq)
        bases = bases[::-1]
    ...
    if type(seq) == list:
        assert len(seq) > 0
        bases = seq[::-1]
        bases = [complement.get(base) for base in bases]
        return bases
...
def design_for_id(args):
    ...
    for in_fasta in args.in_fasta:
        ...
        if args.reversed == '1':  # reversed flag
            for k in seqs.keys():
    ...
    if num_taxa > 1 or specific_against_exists:
        ...
        if args.diff_id_method == "lshnn":
            aq = alignment_query.AlignmentQuerierWithLSHNearNeighbor(alns,
                                                                     args.guide_length, args.diff_id_mismatches,
                                                                     allow_gu_pairs)
    ...
    for i in range(num_aln_for_design):
        ...
        if args.obj == 'minimize-guides':
            ...
            gs = guide_search.GuideSearcherMaximizeActivity(
                ...
                required_flanking_seqs=required_flanking_seqs,
                predictor=predictor,
                do_not_memoize_guides=args.do_not_memoize_guide_computations, ai_model=args.ai_model)
...
def run(args):
    ...
    logger.info("Running design.py with arguments%s: %s", args, type(args))
...
def argv_to_args(argv):
    ...
    # AI model
    base_subparser.add_argument('--ai-model', choices=['cas12a', 'selfCas13', 'adaptCas13'], default='cas12a',
                                help="Predicate objective of a target with different AI model")

    # Amplification mode
    base_subparser.add_argument('--amplification-mode', choices=['PCR', 'LAMP', 'RPA'],
                                default='RPA',
                                help="Amplification mode to increase gene")

    # Reversed
    base_subparser.add_argument('--reversed', choices=['1', '0'],
                                default='0',
                                help="A flag to ponit out if the input fast file should reverse or not. 1: True, "
                                     "0: False")
    ...
    input_autoargs_subparser.add_argument('--metadata-filter', nargs='+',
                                          help=(
                                              "Only include accessions of specified taxonomic ID that match this metadata "
    ...
    input_autoargs_subparser.add_argument('--specific-against-metadata-filter', nargs='+',
                                          help=(
                                              "Only include accessions of the specified taxonomic ID that do not match this "
